...
`output/column/*.md` は独自Markdownで、`src/pages/column/[slug].astro` のレンダラが次の2記法を解釈する（commit 77ade3b で導入、公開済み全記事で使用中）。

- `**テキスト**` → `<strong>`（色 #052c66・font-weight 900・下線なし）。数値・定義・結論に使う。
- `==テキスト==` → `<mark class="marker">`（黄色ハイライト）。判断や注意の一文に使う。
- **太字とマーカーは併用しない**（`**` の中に `==`、`==` の中に `**` を入れない）。
- 本文中の内部リンクは `関連記事：[タイトル](/column/slug/)` を単独行に置く。

`work/seo-contents-creator_プロンプト最適化版.md` には「`**` による強調を使わない」という指示があるが、**本プロジェクトでは適用しない**。`**` はMarkdownの強調ではなく `<strong>` を出すための独自記法なので、削除するとサイト内で本記事だけ装飾が消える。Codexへ依頼する際は `work/column-review-ready.md` にこの旨を明記すること。
